...
const ProjectDetail = () => {
  ...
  const screenshotsRefs = useMemo(() => {
    ...
    return project.details.screenshots.map(() =>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {project.details?.screenshots && (
          <div className="mt-16">
            ...
            {project.details.screenshots.map((screenshot, idx) => {
              const { ref, isVisible, firstLoading } = screenshotsRefs[idx];
        ...
        {project.details?.screenshots && (
          <div className="mt-16">
            ...
            <li className="font-semibold mb-8">
              Next.js와 TypeScript를 활용한 SSR 구현으로 SEO 최적화 및 Tailwind
              CSS를 이용한 효율적인 반응형 디자인 적용
            </li>
            ...
            <li className="font-semibold mb-16">
              GSAP 스크롤 애니메이션과 SSR 간의 충돌 문제: Next.js의 SSR
              환경에서 GSAP 애니메이션이 초기 렌더링 시 오작동
            </li>

            <h3 className="font-bold text-xl">문제 접근</h3>
            <li className="font-semibold mb-16">
              사용자 경험 저하를 방지하기 위해 CSR에서만 GSAP 애니메이션을
              초기화하는 방안 모색
            </li>
            ...
            <li className="font-semibold mb-8">
              ...
              GSAP 라이브러리의 ScrollTrigger.refresh() 메서드를 통해 페이지
              로드 후, 애니메이션 재작동 <br /> ➡️ 조건부 렌더링을 적용하여
              SSR에서는 정적 레이아웃을 제공함으로써 부드러운 스크롤 기반
              애니메이션을 구현하고, <br />
              초기 로딩 시 사용자 경험을 개선
            </li>
            <p className="font-bold text-xl">
              ✅ 이 과정을 통한 웹사이트 성능 테스트: 페이지의 초기 로딩 속도
              20% 개선 + <br /> 유저테스트 피드백에서 긍정적인 반응
            </p>
          </div>
        )}
      </div>
